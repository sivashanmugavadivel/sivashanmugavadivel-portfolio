...
export default function PostCard({ slug, frontmatter }) {
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 700, color: 'var(--text-h)', textAlign: 'center', lineHeight: 1.35 }}>{title}</h3>
            <time style={{ fontSize: '0.75rem', color: 'var(--text)', opacity: 0.6 }}>{formatDate(date)}</time>
          ...
          <Link to={`/blog/${slug}`} style={{
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {tags.map(tag => (
                <span key={tag} style={{ fontSize: '0.68rem', fontWeight: 700, padding: '3px 9px', borderRadius: 999, background: 'rgba(255,255,255,0.25)', color: '#fff' }}>{tag}</span>
              ))}
            </div>
            <p style={{ margin: 0, fontSize: '0.85rem', color: 'rgba(255,255,255,0.92)', lineHeight: 1.6, display: '-webkit-box', WebkitLineClamp: 4, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{excerpt}</p>
  ...
  /* ── Mobile: tap to reveal slide-up overlay (no 3D, no overflow conflict) ── */
  return (
    <div
      style={{
        height: 280, borderRadius: 16, overflow: 'hidden', cursor: 'pointer', position: 'relative',
        background: `linear-gradient(145deg, var(--card-bg) 0%, ${color}11 100%)`,
        border: `1px solid ${color}33`,
        boxShadow: `0 4px 24px ${color}18`,
      }}
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ fontSize: 48, lineHeight: 1, filter: `drop-shadow(0 4px 16px ${color}88)` }}>{icon}</div>
        <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 700, color: 'var(--text-h)', textAlign: 'center', lineHeight: 1.35 }}>{title}</h3>
        <time style={{ fontSize: '0.75rem', color: 'var(--text)', opacity: 0.6 }}>{formatDate(date)}</time>
        <div style={{ position: 'absolute', bottom: 14, right: 16, fontSize: '0.7rem', color, opacity: 0.6, fontWeight: 600 }}>
          tap to reveal ↻
      ...
      <AnimatePresence>
        {flipped && (
          ...
          >
            ...
            <p style={{ margin: 0, fontSize: '0.82rem', color: 'rgba(255,255,255,0.92)', lineHeight: 1.6, display: '-webkit-box', WebkitLineClamp: 4, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{excerpt}</p>
            <Link
              to={`/blog/${slug}`}
